 ...
 * @param x
 * @param y
 * @param replaceTemplates
 * @returns {Promise<*>}
 */
const mergeFiles = async (x, y, replaceTemplates = []) => {
  let X = await fs.readJson(x)

  const Y = typeof y === 'string' ? await fs.readJson(y) : y
  const mergedObject = merge(X, Y)

  if (replaceTemplates.length) {
    let updateTemplateOnmergedObject = JSON.stringify(Object.assign({}, mergedObject))
    replaceTemplates.map(data => {
      updateTemplateOnmergedObject = updateTemplateOnmergedObject.replace(new RegExp(data.pattern, 'g'), data.value)
    })
    return fs.writeJson(x, JSON.parse(updateTemplateOnmergedObject), { replacer: null, spaces: 2 })
  } else {
    return fs.writeJson(x, mergedObject, { replacer: null, spaces: 2 })
  }
}
