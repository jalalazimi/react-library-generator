...
module.exports = async data => {
...

  {
    ...
    ora.promise(promise, '[2/5] \u{1F52C} Test configurations..')
    await promise
  }
  {
    const promise = configParser(path.join(source, 'documentation', docs), name)
    ora.promise(promise, '[3/5] \u{1F4D6} Documentation configurations..')
    await promise
  }

  {
    const promise = mergeFiles(path.join(dest, name, 'package.json'), { name, description, author })
    ora.promise(promise, '[4/5]  \u{2699} Project configurations..')
    await promise
  }
